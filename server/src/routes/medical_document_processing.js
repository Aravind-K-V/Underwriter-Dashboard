...
router.get('/test', (req, res) => {
  ...
  res.json({
    ...
    availableRoutes: [
      ...
      'GET /health-score/:proposer_id',
      'GET /health-score-stored/:proposer_id'
    ]
  });
});
...
router.get('/health-score/:proposer_id', async (req, res) => {
  const { proposer_id } = req.params;

  console.info('[MedicalProcessing][HealthScore] Health score calculation request received:', { proposer_id });
  prettyLog('Health score calculation request received', { proposer_id }, { level: 'info' });

  if (!proposer_id) {
    console.warn('[MedicalProcessing][HealthScore] Missing proposer_id parameter');
  ...
  try {
    ...
    if (proposerResult.rows.length === 0) {
      console.warn('[MedicalProcessing][HealthScore] Proposer not found:', { proposer_id });
    ...
    if (!proposal_number) {
      console.warn('[MedicalProcessing][HealthScore] No proposal found for proposer:', { proposer_id });
      return res.status(400).json({ error: 'No proposal found for this proposer' });
    }

    // Get medical documents with extracted data (flexible document type matching)
    const documentsQuery = `
      SELECT 
        d.id,
        d.document_type,
      ...
      WHERE pr.proposer_id = $1 
        AND (
          d.document_type ILIKE '%medical%' 
          OR d.document_type ILIKE '%health%'
          OR d.document_type ILIKE '%report%'
          OR d.document_type = 'medical_report'
          OR d.document_type = 'medical-report'
          OR d.document_type = 'medical report'
        )
    ...
    const documentsResult = await pool.query(documentsQuery, [proposer_id]);

    if (documentsResult.rows.length === 0) {
      // Check if there are any documents at all for this proposer
      const allDocsQuery = `
        SELECT d.id, d.document_type, d.extracted_data IS NOT NULL as has_extracted_data
        FROM documents d
        LEFT JOIN proposal pr ON d.proposal_number = pr.proposal_number
        WHERE pr.proposer_id = $1
        ORDER BY d.created_at DESC
      `;
      const allDocsResult = await pool.query(allDocsQuery, [proposer_id]);
      
      console.warn('[MedicalProcessing][HealthScore] No processed medical documents found for proposer:', { 
        proposer_id, 
        total_documents: allDocsResult.rows.length,
        documents: allDocsResult.rows.map(doc => ({
          id: doc.id,
          type: doc.document_type,
          has_data: doc.has_extracted_data
        }))
      });
      
      return res.status(400).json({ 
        error: 'No processed medical documents found for this proposer',
        debug_info: {
          total_documents: allDocsResult.rows.length,
          documents: allDocsResult.rows.map(doc => ({
            id: doc.id,
            type: doc.document_type,
            has_extracted_data: doc.has_extracted_data
          }))
        }
      });
    }

    // Prepare documents data for health score calculation
    const documents = documentsResult.rows.map(doc => {
      ...
      try {
        ...
        console.warn('[MedicalProcessing][HealthScore] Failed to parse extracted_data for document:', { 
          document_id: doc.id, 
          error: parseError.message 
        });
        return null;
      ...
      console.debug('[MedicalProcessing][HealthScore] Document extracted data structure:', {
        document_id: doc.id,
        document_type: doc.document_type,
        has_results: !!(extractedData && extractedData.results),
        results_count: extractedData && extractedData.results ? extractedData.results.length : 0,
        data_keys: extractedData ? Object.keys(extractedData) : []
      });
    ...
    if (documents.length === 0) {
      console.warn('[MedicalProcessing][HealthScore] No valid medical documents found after parsing:', { proposer_id });
      return res.status(400).json({ error: 'No valid medical document data found' });
    }

    console.debug('[MedicalProcessing][HealthScore] Documents prepared for health score calculation:', {
      proposer_id,
    ...
    const healthData = {
      ...
      gender: proposerData.sex || 'male',
      age: proposerData.age || 30,
      documents: documents
    };

    console.debug('[MedicalProcessing][HealthScore] Prepared health data:', {
      proposer_id,
      gender: healthData.gender,
      age: healthData.age,
      documents_count: documents.length,
      sample_document: documents[0] ? {
        id: documents[0].document_id,
        type: documents[0].document_type,
        has_data: !!documents[0].extracted_data
      } : null
    });
    prettyLog('Prepared health data for calculation', {
      proposer_id,
      gender: healthData.gender,
      age: healthData.age,
      documents_count: documents.length
    ...
    // Calculate health score using Python script
    console.debug('[MedicalProcessing][HealthScore] Calling Python script with data:', JSON.stringify(healthData, null, 2).substring(0, 1000));
    const healthScore = await calculateHealthScore(healthData);

    if (!healthScore) {
      console.error('[MedicalProcessing][HealthScore] Health score calculation failed - no results returned');
      return res.status(500).json({ error: 'Health score calculation failed' });
    }

    // Store the health score result in risk_assessments table
    try {
      // First check if a risk assessment already exists for this request
      ...
      `;
      const existingResult = await pool.query(existingAssessmentQuery, [proposer_id]);

      if (existingResult.rows.length > 0) {
        // Update existing assessment
        const request_id = existingResult.rows[0].request_id;
        const updateQuery = `
          UPDATE risk_assessments 
          SET medical_score = $1, updated_at = NOW()
          WHERE request_id = $2
        `;
        await pool.query(updateQuery, [healthScore.percentage_score, request_id]);
        console.debug('[MedicalProcessing][HealthScore] Updated existing health score in risk_assessments');
      } else {
        console.warn('[MedicalProcessing][HealthScore] No existing risk assessment found to update health score');
      }
    } catch (dbError) {
      console.warn('[MedicalProcessing][HealthScore] Failed to store health score in database:', dbError.message);
    ...
    const response = {
      ...
      proposer_id: parseInt(proposer_id),
      proposal_number: parseInt(proposal_number),
      health_score: healthScore.percentage_score,
      total_score: healthScore.total_score,
      max_possible_score: healthScore.max_possible_score,
      risk_level: healthScore.risk_level,
      red_flags: healthScore.red_flags,
      recommendations: healthScore.recommendations[0],
      test_results: healthScore.test_results,
      evaluation_metadata: healthScore.evaluation_metadata,
      calculated_at: new Date().toISOString()
    };

    console.info('[MedicalProcessing][HealthScore] Health score calculation completed successfully:', { 
      proposer_id, 
      health_score: healthScore.percentage_score,
      risk_level: healthScore.risk_level 
    });
    prettyLog('Health score calculation completed successfully', { 
      proposer_id, 
      health_score: healthScore.percentage_score,
      risk_level: healthScore.risk_level 
    }, { level: 'info' });
    ...
    console.error('[MedicalProcessing][HealthScore] Health score calculation failed:', error.message);
    prettyLog('Health score calculation failed', { error: error.message }, { level: 'error' });
    res.status(500).json({ error: 'Internal server error', details: error.message });
  }
});

// Get stored health score from risk_assessments table
router.get('/health-score-stored/:proposer_id', async (req, res) => {
  const { proposer_id } = req.params;

  console.info('[MedicalProcessing][HealthScoreStored] Fetching stored health score:', { proposer_id });
  prettyLog('Fetching stored health score', { proposer_id }, { level: 'info' });

  if (!proposer_id) {
    console.warn('[MedicalProcessing][HealthScoreStored] Missing proposer_id parameter');
    return res.status(400).json({ error: 'Proposer ID is required' });
  }

  try {
    // Get the stored health score from risk_assessments table
    ...
    if (storedScoreResult.rows.length === 0) {
      console.info('[MedicalProcessing][HealthScoreStored] No stored health score found');
      return res.status(404).json({ error: 'No stored health score found' });
    }

    const storedScore = storedScoreResult.rows[0];

    // Check if medical_score is null or undefined
    if (storedScore.medical_score === null || storedScore.medical_score === undefined) {
      console.info('[MedicalProcessing][HealthScoreStored] Stored health score is null, triggering calculation');
      return res.status(404).json({ error: 'No valid stored health score found - medical_score is null' });
    }

    // Map outcome back to risk level
    let risk_level = 'Unknown';
    if (storedScore.outcome === 'approved') {
      risk_level = storedScore.medical_score >= 80 ? 'Low' : 'Medium';
    } else if (storedScore.outcome === 'reject') {
      risk_level = 'High';
    } else if (storedScore.outcome === 'pending') {
      risk_level = 'Medium';
    }
    ...
    const response = {
      ...
      health_score: storedScore.medical_score,
    ...
    console.info('[MedicalProcessing][HealthScoreStored] Stored health score retrieved successfully:', { 
      proposer_id, 
      health_score: storedScore.medical_score 
    });
    prettyLog('Stored health score retrieved successfully', { 
      proposer_id, 
      health_score: storedScore.medical_score 
    }, { level: 'info' });
    ...
    console.error('[MedicalProcessing][HealthScoreStored] Failed to fetch stored health score:', error.message);
    prettyLog('Failed to fetch stored health score', { error: error.message }, { level: 'error' });
    res.status(500).json({ error: 'Internal server error', details: error.message });
  }
});

// Helper function to calculate health score using Python script
async function calculateHealthScore(healthData) {
  return new Promise((resolve, reject) => {
    const __filename = fileURLToPath(import.meta.url);
    const __dirname = path.dirname(__filename);
    
    // Path to the Python script
    const scriptPath = path.join(__dirname, '..', 'Rule Engines', 'Health Score', 'Rule Engine', 'calculate_single_score.py');
    
    console.debug('[MedicalProcessing][HealthScore] Executing Python script:', { scriptPath });
    ...
    pythonProcess.on('close', (code) => {
      console.debug('[MedicalProcessing][HealthScore] Python script finished:', { 
        code, 
      ...
      if (code !== 0) {
        console.error('[MedicalProcessing][HealthScore] Python script failed:', { code, error: errorOutput });
        reject(new Error(`Health score calculation failed with code ${code}: ${errorOutput}`));
        return;
      }

      if (!output.trim()) {
        console.error('[MedicalProcessing][HealthScore] Python script returned empty output');
        reject(new Error('Health score calculation returned empty result'));
        return;
      }

      try {
        const result = JSON.parse(output.trim());
        console.debug('[MedicalProcessing][HealthScore] Python script completed successfully:', {
          has_proposer_id: !!result.proposer_id,
          has_percentage_score: !!result.percentage_score,
          has_risk_level: !!result.risk_level
        });
        resolve(result);
      } catch (parseError) {
        console.error('[MedicalProcessing][HealthScore] Failed to parse Python script output:', { 
          parseError: parseError.message,
          output: output.substring(0, 1000)
        });
        reject(new Error(`Failed to parse health score result: ${parseError.message}`));
      }
    });

    pythonProcess.on('error', (error) => {
      console.error('[MedicalProcessing][HealthScore] Failed to start Python process:', error.message);
      reject(new Error(`Failed to start health score calculation: ${error.message}`));
    });

    // Send health data to Python script
    try {
      pythonProcess.stdin.write(JSON.stringify(healthData));
      pythonProcess.stdin.end();
    } catch (error) {
      console.error('[MedicalProcessing][HealthScore] Failed to send data to Python script:', error.message);
      reject(new Error(`Failed to send data to health score calculation: ${error.message}`));
    }
  });
}
