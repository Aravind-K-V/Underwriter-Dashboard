...
// PATCH endpoint for Escalating status to Head
router.patch('/underwriting-requests/:proposer_id/escalate', async (req, res) => {
  const { proposer_id } = req.params;
  const { status, message, underwriter_email, head_email } = req.body;

  console.info('[Underwriting][StatusUpdate] PATCH request received for proposer:', { proposer_id, status, message, underwriter_email, head_email });
  ...
  const validStatuses = ['Escalated to Head'];
  ...
  try {
    ...
    if (checkResult.rows.length === 0) {
      ...
      const insertQuery = `
        INSERT INTO underwriting_requests (proposer_id, status, message, assigned_to_email, created_at, updated_at)
        VALUES ($1, $2, $3, $4, NOW(), NOW())
        RETURNING *
      `;

      const insertResult = await pool.query(insertQuery, [proposer_id, status, message || null, underwriter_email]);
      ...
      const updateQuery = `
        UPDATE underwriting_requests
        SET status = $1, message = $2, assigned_to_email = $3, escalated_by = $4, escalated_to_head = true, escalated_at = NOW(), updated_at = NOW()
        WHERE proposer_id = $5
        RETURNING *
      `;
      
      const updateResult = await pool.query(updateQuery, [status, message || null, head_email, underwriter_email, proposer_id]);
