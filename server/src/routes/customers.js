...
router.get('/customers', async (req, res) => {
  ...
  try {
    ...
    if (status && status !== 'All Applications') {
      ...
      } else if (status === 'Escalated Requests') {
        // Show only escalated requests where escalated_to_head is true
        query += ' AND ur.escalated_to_head = true AND ur.status = $1';
        params.push('Escalated to Head');
      } else {
