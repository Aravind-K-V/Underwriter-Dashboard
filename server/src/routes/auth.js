...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    const userCheck = await pool.query(
      'SELECT * FROM user_login WHERE email_id = $1 AND (role = $2 OR role = $3) AND status = $4',
      [email_id, 'underwriter', 'underwriter_head', 'approved']
    );
...
router.post('/reset-password', async (req, res) => {
  ...
  try {
    ...
    const updateResult = await pool.query(
      'UPDATE user_login SET password = $1 WHERE email_id = $2 AND (role = $3 OR role = $4) AND status = $5 RETURNING email_id',
      [hashedPassword, email_id, 'underwriter', 'underwriter_head', 'approved']
    );
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (user.role !== 'underwriter' && user.role !== 'underwriter_head' || user.status !== 'approved') {
...
router.get('/current-user/:email_id', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'SELECT email_id, name, role, status, created_date FROM user_login WHERE email_id = $1 AND (role = $2 OR role = $3) AND status = $4',
      [email_id, 'underwriter', 'underwriter_head', 'approved']
    );
