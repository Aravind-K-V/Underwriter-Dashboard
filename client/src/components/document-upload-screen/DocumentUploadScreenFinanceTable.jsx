...
const DocumentUploadScreenFinanceTable = ({ activeTab, setActiveTab, bothTabsPresent, reviewFlags }) => {
  ...
  const handleEscalateToHead = async (message) => {
    setStatusUpdateLoading(true);
    try {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div className={`text-white font-medium ${isDesktop ? 'text-xs' : 'text-xs'}`}>
            Financial Risk
          </div>
        </div>
              ...
              {reviewFlags?.mc_required === false ? (
                <>
                  ...
                  <button
                    ...
                    disabled={(() => {
                      const user = JSON.parse(localStorage.getItem('user') || '{}');
                      const isHead = user.role === 'underwriter_head';
                      return statusUpdateLoading || currentStatus === 'Escalated to Head' || isHead;
                    })()}
                    style={{
                      minWidth: 140,
                      height: 'auto',
                      ...
                      background: (() => {
                        const user = JSON.parse(localStorage.getItem('user') || '{}');
                        const isHead = user.role === 'underwriter_head';
                        if (currentStatus === 'Escalated to Head' || isHead) return '#E2EAFB';
                        return '#0252A9';
                      })(),
                      color: (() => {
                        const user = JSON.parse(localStorage.getItem('user') || '{}');
                        const isHead = user.role === 'underwriter_head';
                        if (currentStatus === 'Escalated to Head' || isHead) return '#0252A9';
                        return 'white';
                      })(),
                      borderRadius: 12,
                      border: 'none',
                      ...
                      cursor: (() => {
                        const user = JSON.parse(localStorage.getItem('user') || '{}');
                        const isHead = user.role === 'underwriter_head';
                        return (statusUpdateLoading || currentStatus === 'Escalated to Head' || isHead) ? 'not-allowed' : 'pointer';
                      })(),
                      opacity: (() => {
                        const user = JSON.parse(localStorage.getItem('user') || '{}');
                        const isHead = user.role === 'underwriter_head';
                        return (statusUpdateLoading || currentStatus === 'Escalated to Head' || isHead) ? 0.7 : 1;
                      })(),
                      transition: 'all 0.2s',
                      whiteSpace: 'normal',
                      wordBreak: 'break-word',
              ...
              {approvalModalStatus === 'confirm' && (
                <>
                  ...
                  <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 8 }}>
                    ...
                    {approvalModalType === 'escalate' && 'Confirm Escalation to Head?'}
                  </div>
                  <div style={{ color: '#666', fontSize: 15, marginBottom: 16 }}>
                    ...
                    {approvalModalType === 'escalate' && 'Please provide notes for escalation to the head:'}
                  </div>
                    ...
                    placeholder={
                      approvalModalType === 'approve' ? 'Enter approval reason...' :
                        approvalModalType === 'reject' ? 'Enter rejection reason...' :
                          approvalModalType === 'investigate' ? 'Enter investigation notes...' :
                            approvalModalType === 'escalate' ? 'Enter escalation notes...' :
                              ''
                    }
                    required
                    style={{
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          if (approvalModalType === 'escalate') {
                            await handleEscalateToHead(modalMessage.trim());
