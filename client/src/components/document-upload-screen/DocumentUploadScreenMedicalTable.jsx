...
const DocumentUploadScreenMedicalTable = ({ activeTab, setActiveTab, bothTabsPresent, reviewFlags }) => {
  ...
  // Health Score states (similar to finance score implementation)
  ...
  const healthMetrics = [
    ...
    {
      id: 2,
      title: 'Kidney Function Tests (KFT) or Renal Function Tests (RFT)',
      subParameters: [
        'SERUM CREATININE',
        'Blood Urea',
        'Uric Acid',
  ...
  // Helper function to determine health risk level based on health score response
  const getHealthRiskLevel = (healthScoreOrPercentage) => {
    // If healthScoreOrPercentage is an object (full health score response), prioritize risk_level
    if (typeof healthScoreOrPercentage === 'object' && healthScoreOrPercentage !== null) {
      // Use the backend's risk_level which includes red flag detection
      if (healthScoreOrPercentage.risk_level) {
        return healthScoreOrPercentage.risk_level.toLowerCase();
      }
      // Fallback to percentage calculation if risk_level is not available
      const percentage = healthScoreOrPercentage.health_score || healthScoreOrPercentage.percentage_score;
      if (percentage !== null && percentage !== undefined) {
        return calculateRiskFromPercentage(percentage);
      }
      return 'unknown';
    }

    // If it's just a number (percentage), calculate risk level
    if (typeof healthScoreOrPercentage === 'number') {
      return calculateRiskFromPercentage(healthScoreOrPercentage);
    }

    return 'unknown';
  };

  // Helper function to calculate risk level from percentage only
  const calculateRiskFromPercentage = (percentage) => {
    if (!percentage || percentage < 0) return 'unknown';
    if (percentage > 80) return 'safe';
    if (percentage >= 60) return 'low_risk';
    if (percentage >= 40) return 'medium_risk';
    if (percentage >= 20) return 'high_risk';
    return 'critical';
  };

  // Helper function to format risk level for display (remove underscores and capitalize)
  const formatRiskLevelForDisplay = (riskLevel) => {
    if (!riskLevel) return 'Unknown';

    // Handle different risk level formats
    const cleanLevel = riskLevel.toString().toLowerCase();
    ...
    switch (cleanLevel) {
      ...
      case 'unknown':
        return 'Unknown';
      // Handle existing format (already formatted)
      case 'low':
        return 'Low';
      case 'medium':
        return 'Medium';
  ...
  };  //  Function to fetch health score (stored or calculate new)
  const fetchHealthScore = async () => {
    if (!proposer_id) {
      console.debug('[DocumentUpload][MedicalTable] No proposer_id available');
      setHealthScore(null);
      return;
    }
    ...
    try {
      ...
      // First try to get stored health score - Implement it Later
      // let response = await fetch(`${NODE_API_URL}/api/medical-document-processing/health-score-stored/${proposer_id}`);

      // if (response.ok) {
      //   const scoreData = await response.json();
      //   // Check if the stored score is valid (not null)
      //   if (scoreData.health_score !== null && scoreData.health_score !== undefined) {
      //     setHealthScore(scoreData);
      //     console.debug('[DocumentUpload][MedicalTable] Health score retrieved from storage:', scoreData);
      //     return;
      //   } else {
      //     console.debug('[DocumentUpload][MedicalTable] Stored health score is null, calculating new score');
      //   }
      // } else {
      //   console.debug('[DocumentUpload][MedicalTable] No stored health score found, calculating new score');
      // }

      // If no stored score or stored score is null, calculate new one
      let score_response = await fetch(`${NODE_API_URL}/api/medical-document-processing/health-score/${proposer_id}`);

      if (score_response.ok) {
        const scoreData = await score_response.json();
        setHealthScore(scoreData);
        console.debug('[DocumentUpload][MedicalTable] Health score calculated:', scoreData);
      } else {
        const errorData = await score_response.json();
        setHealthScoreError(`Failed to calculate health score: ${score_response.status}`);
        console.error('[DocumentUpload][MedicalTable] Health score calculation failed:', errorData);

        // If calculation fails, show debug info if available
        if (errorData.debug_info) {
          console.debug('[DocumentUpload][MedicalTable] Debug info:', errorData.debug_info);
        }
      }
    } catch (error) {
      console.error('[DocumentUpload][MedicalTable] Health score fetch error:', error);
      setHealthScoreError(error.message);
    } finally {
  ...
  //  Function to handle escalation to head
  const handleEscalateToHead = async (newStatus, message) => {
    setStatusUpdateLoading(true);
    try {
      console.info('[DocumentUpload][FinanceTable] Escalating proposal to head:', proposer_id);
      const user = JSON.parse(localStorage.getItem('user'));
      const response = await fetch(`${NODE_API_URL}/api/underwriting/underwriting-requests/${proposer_id}/escalate`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          status: newStatus,
          underwriter_email: user.email_id || 'unknown',
          head_email: user.head_email || 'anikethgv03@gmail.com', // Needs to be chnaged later
          message: message
        })
      });

      if (!response.ok) {
        throw new Error(`Failed to mark for Escalation: ${response.statusText}`);
      }

      const result = await response.json();
      console.info('[DocumentUpload][FinanceTable] Proposal marked for Escalation successfully:', { hasData: !!result });

      // Trigger header refresh
      window.dispatchEvent(new CustomEvent('statusUpdated', {
        detail: { proposer_id, status: 'Escalated to Head' }
      }));

    } catch (error) {
      console.error('[DocumentUpload][FinanceTable] Error marking for Escalation:', error.message);
      throw error;
    } finally {
      setStatusUpdateLoading(false);
    }
  ...
  const handleConfirmClick = async () => {
    ...
    try {
      ...
      if (modalType === 'approve') {
        ...
        await updateUnderwritingStatus('Escalated to Head', modalMessage.trim());
        newStatus = 'Escalated to Head';
      }
  ...
  const calculateOverallHealthScore = () => {
    ...
    filteredMetrics.forEach(metric => {
      ...
      if (metricData && metricData.presentParameters) {
        ...
        // Count parameters that are in normal range and update their status
        presentParams.forEach(param => {
          const isInRange = isParameterInRange(param.value, param.referenceRange);

          if (param.status === 'Normal' || param.status === 'normal' || isInRange) {
  ...
  const isParameterInRange = (value, referenceRange) => {
    if (!value || value === 'N/A' || !referenceRange) return false;
    ...
    try {
      ...
      if (typeof referenceRange === 'object' && referenceRange !== null) {
        if (referenceRange.Normal) {
          rangeString = String(referenceRange.Normal);
        } else if (referenceRange['Reference Range']) {
          rangeString = String(referenceRange['Reference Range']);
        } else {
          console.debug('Unknown reference range format:', referenceRange);
          return false; // Changed to false for unknown formats
        }
      } else {
      ...
      // Pattern 1: Simple range (e.g., "4.5-11.0")
      ...
      // Pattern 2: Less than range (e.g., "< 5.5-6.9")
      const lessThanRange = rangeString.match(/^<\s*(\d+(?:\.\d+)?)\s*-\s*(\d+(?:\.\d+)?)$/);
      if (lessThanRange) {
        const upperLimit = parseFloat(lessThanRange[2]);
        return numValue <= upperLimit;
      }

      // Pattern 3: Zero-based range (e.g., "0-11")
      const zeroRange = rangeString.match(/^0\s*-\s*(\d+(?:\.\d+)?)$/);
      if (zeroRange) {
        const max = parseFloat(zeroRange[1]);
        return numValue >= 0 && numValue <= max;
      }

      // Pattern 4: Simple less than (e.g., "< 5.5")
      ...
      // Pattern 5: Greater than or equal (e.g., ">= 12")
      const greaterEqual = rangeString.match(/^>=\s*(\d+(?:\.\d+)?)$/);
      if (greaterEqual) {
        const min = parseFloat(greaterEqual[1]);
        return numValue >= min;
      }

      // Pattern 6: Greater than (e.g., "> 10")
      const greaterThan = rangeString.match(/^>\s*(\d+(?:\.\d+)?)$/);
      if (greaterThan) {
        const min = parseFloat(greaterThan[1]);
        return numValue > min;
      }

      console.debug('Unmatched range pattern:', rangeString, 'for value:', numValue);
      return false; // Changed to false for unmatched patterns
    } catch (error) {
      console.error('Error in isParameterInRange:', error);
      return false; // Changed to false on error
    }
  };
  ...
  const getNormalParameterCount = (metricId) => {
    ...
    return metricData.presentParameters.filter(param => {
      // First check existing status, then verify with range checking
      const isInRange = isParameterInRange(param.value, param.referenceRange);
      return param.status === 'Normal' || param.status === 'normal' || isInRange;
    }).length;
  };
  ...
  const handleProcessAllDocuments = async () => {
    ...
    try {
      ...
      // Fetch health score after all documents are processed
      setTimeout(() => {
        fetchHealthScore();
      }, 1000);
  ...
  // Fetch health score when proposer_id changes
  useEffect(() => {
    fetchHealthScore();
  }, [proposer_id]);
  ...
  // Update parameter statuses when health analysis changes
  useEffect(() => {
    if (processedHealthAnalysis) {
      // Trigger a recalculation to update parameter statuses
      calculateOverallHealthScore();
      console.debug('[DocumentUpload][MedicalTable] Parameter statuses updated based on health analysis');
    }
  }, [processedHealthAnalysis]);
    ...
    >
      ...
      <div className={`${isDesktop ? 'flex gap-4 flex-1 overflow-hidden' : 'flex flex-col gap-3 flex-1'}`}>
        ...
        <div className={`${isDesktop ? 'flex-1' : 'w-full'} flex flex-col gap-3 relative z-10`}>
          ...
          <div
            className={`relative w-full rounded-xl overflow-hidden flex justify-between items-start gap-3 flex-shrink-0 z-20 ${isDesktop ? 'h-28 p-3' : isTablet ? 'h-24 p-3' : 'h-20 p-2'
              }`}
            style={{
          ...
          >
            ...
            <div className="relative z-10 w-full h-full flex flex-col justify-between">
              <div className="flex items-center justify-between w-full">
                {/* Left side - Icon and Health Score */}
                <div className="flex items-center gap-3">
                  <div className={`bg-gradient-to-br from-blue-600 to-blue-400/40 rounded-full flex items-center justify-center ${isDesktop ? 'w-10 h-10' : 'w-9 h-9'
                    }`}>
                    <img src={healthScoreIcon} alt="Health Score" className={isDesktop ? 'w-6 h-6' : 'w-5 h-5'} />
                  </div>
                  <div className="flex flex-col">
                    ...
                    <div className="flex items-end gap-1">
                      {healthScoreLoading ? (
                        ...
                        <span className={`text-red-300 font-medium ${isDesktop ? 'text-xs' : 'text-xs'}`}>
                          Error loading
                        </span>
                      ) : healthScore && healthScore.health_score !== null && healthScore.health_score !== undefined ? (
                        <>
                          <span className={`text-[#8FB1F8] font-medium ${isDesktop ? 'text-2xl' : isTablet ? 'text-xl' : 'text-lg'}`}>
                            {parseFloat(healthScore.health_score).toFixed(1)}
                          </span>
                          <span className={`text-white font-medium ${isDesktop ? 'text-2xl' : isTablet ? 'text-xl' : 'text-lg'}`}>/</span>
                          <span className={`text-[#8FB1F8] font-medium ${isDesktop ? 'text-2xl' : isTablet ? 'text-xl' : 'text-lg'}`}>100.0</span>
                        </>
                      ) : (
                        <>
                          <span className={`text-[#8FB1F8] font-medium ${isDesktop ? 'text-2xl' : isTablet ? 'text-xl' : 'text-lg'}`}>
                ...
                <div className="flex items-center mt-7">
                  {healthScore && healthScore.health_score !== null && healthScore.health_score !== undefined ? (
                    <div style={{
                      ...
                      paddingBottom: 8,
                      background: (() => {
                        const riskLevel = getHealthRiskLevel(healthScore);
                        if (riskLevel === 'safe') return 'rgba(45, 210, 154, 0.8)';
                        // if (riskLevel === 'low_risk') return 'rgba(209, 189, 4, 1)';
                        if (riskLevel === 'low_risk') return 'rgba(235, 180, 2, 1)';
                        if (riskLevel === 'medium_risk') return 'rgba(255, 149, 0, 1)';
                        if (riskLevel === 'high_risk') return 'rgba(242, 51, 54, 1)';
                        if (riskLevel === 'critical') return 'rgba(242, 51, 54, 1)';
                        return 'linear-gradient(96deg, #5A98FF 0%, #0463FF 100%)';
                      })(),
                      borderRadius: 8,
                      display: 'inline-flex',
                      alignItems: 'center',
                      ...
                      <span style={{
                        ...
                        {formatRiskLevelForDisplay(getHealthRiskLevel(healthScore))}
                      </span>
                    </div>
                  ) : (
                    <div style={{
                      ...
                      paddingTop: 6,
                      paddingBottom: 6,
                      background: 'linear-gradient(96deg, #5A98FF 0%, #0463FF 100%)',
                      borderRadius: 6,
                      display: 'inline-flex',
                      alignItems: 'center',
                      gap: 5,
              ...
              <div className="flex items-center justify-between w-full mt-2">
                {/* Display recommendations from API health score - only if risk level is critical */}
                {healthScore && healthScore.recommendations && healthScore.recommendations.length > 0 && getHealthRiskLevel(healthScore) === 'critical' && (
                      ...
                      <div className="flex items-center gap-2">
                        {!isMobile && (
                          <div className={`px-2 py-0.5 ${getRiskLevelColor(riskLevel)} rounded flex items-center gap-1`}>
                            <img src={getRiskIcon(riskLevel)} alt={`${formatRiskLevelForDisplay(riskLevel)} Risk`} className="w-2 h-2" />
                            <span className="text-xs font-medium">Risk: {formatRiskLevelForDisplay(riskLevel)}</span>
                          </div>
                        )}
                        <button
                          onClick={(e) => {
                        ...
                        <div className="space-y-1">
                          ...
                          >
                            {presentParams.map((param, idx) => {
                              ...
                              // Determine parameter styles based on range check
                              let paramStyles;
                              if (param.status === "Normal" || param.status === "normal" || isInRange) {
                                paramStyles = 'bg-[#E2EAFB] text-[#0252A9]';
                              } else if (param.status === "Out of Range" || param.status === "out of range" || !isInRange) {
                                paramStyles = 'bg-orange-100 text-orange-800';
                              } else {
                                // Default/unknown status
                                paramStyles = 'bg-gray-100 text-gray-800 border-gray-200';
                              }

                              return (
                                <div
                                  key={idx}
                                  className={`inline-flex flex-col items-center justify-center rounded-lg border-2 ${paramStyles}`}
                                  style={{
                                  ...
                                  title={`${param.name}: ${displayValue} ${param.unit || ''} - Status: ${isParameterInRange(param.value, param.referenceRange) ? 'Normal' : 'Out of Range'}`}
                                >
                                  <span
          ...
          (currentStatus === 'Approved' || currentStatus === 'Rejected') ? (
            ...
            >
              ...
              <button
                ...
                disabled={statusUpdateLoading || currentStatus === 'Escalated to Head'}
                style={{
                  minWidth: 140,
                  height: 'auto',
                  minHeight: isMobile ? 34 : isTablet ? 36 : isDesktop ? 38 : 40,
                  padding: '8px 12px',
                  background: currentStatus === 'Escalated to Head' ? '#E2EAFB' : '#0252A9',
                  color: currentStatus === 'Escalated to Head' ? '#0252A9' : 'white',
                  borderRadius: 12,
                  border: 'none',
                  fontFamily: 'PP Neue Montreal, "PP Neue Montreal", sans-serif',
                  fontWeight: 500,
                  fontSize: isMobile ? 12 : isTablet ? 13 : isDesktop ? 14 : 15,
                  boxShadow: '0 4px 12px rgba(5, 82, 169, 0.18)',
                  cursor: (statusUpdateLoading || currentStatus === 'Escalated to Head') ? 'not-allowed' : 'pointer',
                  opacity: (statusUpdateLoading || currentStatus === 'Escalated to Head') ? 0.7 : 1,
                  transition: 'all 0.2s',
                  whiteSpace: 'normal',
              ...
              >
                {statusUpdateLoading && currentStatus !== 'Escalated to Head'
                  ? 'Processing...'
                  : 'Escalate to Head'}
              </button>
              ...
              {modalStatus === 'confirm' && (
                <>
                  ...
                  <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 8 }}>
                    ...
                    {modalType === 'escalate' && 'Confirm Escalate to Head?'}
                  </div>
                  <div style={{ color: '#666', fontSize: 15, marginBottom: 16 }}>
                    ...
                    {modalType === 'escalate' && 'Please provide notes for escalation to the head:'}
                  </div>
                    ...
                    placeholder={
                      modalType === 'approve' ? 'Enter approval reason...' :
                        modalType === 'reject' ? 'Enter rejection reason...' :
                          modalType === 'investigate' ? 'Enter investigation notes...' :
                            modalType === 'escalate' ? 'Enter escalation notes...' : ''
                    }
                    required
                    style={{
                      width: '100%',
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          if (modalType === 'approve') {
                            ...
                            await handleEscalateToHead('Escalated to Head', modalMessage.trim());
                            newStatus = 'Escalated to Head';
                          }
